...
$previousMonthFirstDay = date("1-m-Y", strtotime("-1 month"));
$totalDaysInMonth = date("t", strtotime($firstDayOfMonth));
$previousMonthTotalDays = date("t", strtotime($previousMonthFirstDay));
...
<br>
<h3>Previous Month Table</h3>
<table border="1" cellspacing="0">
    <?php
    $counter = 0;

    $lastDayOfMonth = date('Y-m-t', strtotime('-1 day', strtotime($previousMonthFirstDay)));

    // Calculate the number of Sundays in the current month
    $sundaysCount = 0;
    for ($j = 0; $j < $previousMonthTotalDays; $j++) {
        $dateOfAttendance = date("Y-m-d", strtotime("$previousMonthFirstDay +$j days"));
        $dayOfWeek = date('N', strtotime($dateOfAttendance));
        if ($dayOfWeek == 7) {
            $sundaysCount++;
        }
    ...
    for ($i = 1; $i <= $totalNumberOfEmployees + 2; $i++) {
        if ($i == 1) {
            echo "<tr>";
            echo "<td rowspan='2'>Names</td>";
            for ($j = 1; $j <= $previousMonthTotalDays; $j++) {
                echo "<td>$j</td>";
            ...
            echo "<tr>";
            for ($j = 0; $j < $previousMonthTotalDays; $j++) {
                echo "<td>" . date("D", strtotime("+$j days", strtotime($previousMonthFirstDay))) . "</td>";
            }
            echo "<td>Present</td>";
            echo "<td>Absent</td>";
            echo "<td>Leaves</td>";
            ...
            $ciCoColumn = mysqli_query($db, "
                ...
                FROM camsbiometricattendance
                WHERE UserID = '" . $EmployeesIDsArray[$counter] . "'
                AND DATE(AttendanceTime) >= '$previousMonthFirstDay'
                AND DATE(AttendanceTime) <= '$lastDayOfMonth'
                AND AttendanceType = 'CheckIn'
                AND DATE(AttendanceTime) IN (
                    SELECT DATE(AttendanceTime)
        ...
        ") or die(mysqli_error($db));
            ...
            $leavesData = mysqli_query($db, "
                ...
                AND status1 = 1
                AND (
                    (`from` BETWEEN '$previousMonthFirstDay' AND '$lastDayOfMonth')
                    OR (`to` BETWEEN '$previousMonthFirstDay' AND '$lastDayOfMonth')
                    OR (`from` <= '$previousMonthFirstDay' AND `to` >= '$lastDayOfMonth')
                )
            ") or die(mysqli_error($db));
            ...
            while ($leaveEntry = mysqli_fetch_assoc($leavesData)) {
                ...
                for ($k = 0; $k <= $to - $from; $k += 24 * 60 * 60) {
                    $currentDay = date('N', $from + $k);
                    if ($currentDay != 7 && $from + $k >= strtotime($previousMonthFirstDay) && $from + $k <= strtotime($lastDayOfMonth)) {
                        $leaveCount++;
                    }
                }
            }
            ...
            // Calculate the number of working days in the current month
            $totalWorkingDays = $previousMonthTotalDays - $sundaysCount;

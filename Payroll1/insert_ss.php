...
for ($i = 0; $i < count($empname); $i++) {
    ...
    $sql = "INSERT INTO payroll_ss (empname,salarymonth, fbp, fhra, foa, fgs, monthdays, present, leaves, sundays, flop, paydays, bp, hra, oa, gross, epf1, epf2, pension, esi1,esi2, tds, emi, lopamt, totaldeduct, payout, misc, bonus, created, status,status1) 
    VALUES ('{$empname[$i]}', '{$salarymonth[$i]}', '{$fbp[$i]}', '{$fhra[$i]}', '{$foa[$i]}', '{$fgs[$i]}', '{$monthdays[$i]}', '{$present[$i]}', '{$leaves[$i]}', '{$sundays[$i]}', '{$flop[$i]}', '{$paydays[$i]}', '{$bp[$i]}', '{$hra[$i]}', '{$oa[$i]}', '{$gross[$i]}', '{$epf1}', '{$epf2}', '{$pension}', '{$esi1[$i]}','{$esi2}', '{$tds[$i]}', '{$emi[$i]}', '{$lopamt[$i]}', '{$totaldeduct[$i]}', '{$payout[$i]}', '{$misc[$i]}', '{$bonus[$i]}', NOW(), '1','0')";

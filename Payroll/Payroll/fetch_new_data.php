...
if(isset($_POST['id'])) {
    ...
    $sql = "UPDATE payroll_ss SET status = 1, status1 = 1, confirm1 = '$created' WHERE id = '$id'";
    if(mysqli_query($con, $sql)) {
        // Status updated successfully
        // Fetch the next available data
        $query = "SELECT * FROM payroll_ss WHERE status = 0 OR status = 1 LIMIT 1";
    ...
    $query = "SELECT * FROM payroll_ss WHERE status = 0 LIMIT 1";
    $result = mysqli_query($con, $query);
    if ($result && mysqli_num_rows($result) > 0) {
